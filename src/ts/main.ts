...
function onUndo(ev: Event, game: Game): void {
    ...
    game.board.board.forEach((row, rowIdx) => row.forEach((cell, colIdx) => {
        if (cell.isShown) Gui.renderCell(cell.htmlStr, { row: rowIdx, col: colIdx })
    }))
...
function handleRevealNeighbors(coords: CoordsModel, game: Game, cell: Cell): void {
    revealNeighbors(coords, game, cell)
    setTimeout(revealNeighbors, 1500, coords, game, cell, '<span> </span>')

}

function revealNeighbors(coords: CoordsModel, game: Game, cell: Cell, htmlStr = ''): void {
