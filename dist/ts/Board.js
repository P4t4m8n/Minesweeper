...
var _Board_instances, _Board_getBombSvg, _Board_getNumberCellContent;
...
export class Board {
    ...
    initializeBoard(size) {
        this._board = Array.from({ length: size }, (_, row) => Array.from({ length: size }, (_, col) => new Cell('<span>0</span>', { row, col })));
    }
    placeMines(minesCount, coords) {
        let placedMines = 0;
        let loopBreaker = 0;
    ...
    placeMine(cell) {
        cell.isMine = true;
        cell.htmlStr = __classPrivateFieldGet(this, _Board_instances, "m", _Board_getBombSvg).call(this);
    }
    countMinesAround() {
        let size = this._board.length;
        for (let row = 0; row < size; row++) {
            for (let col = 0; col < size; col++) {
                if (this._board[row][col].isMine)
                    continue;
                let minesAround = 0;
                ...
                this._board[row][col].htmlStr = __classPrivateFieldGet(this, _Board_instances, "m", _Board_getNumberCellContent).call(this, minesAround);
            }
        }
    }
    neighborsLoop(coords, callback) {
...
}, _Board_getNumberCellContent = function _Board_getNumberCellContent(number) {
    const colors = {
        1: '#0332fe',
        2: '#019f02',
        3: '#ff2600',
